            ...
            <div class="card hoverCard LoginForm">
                <div class="card-body" style="display: flex; justify-content: center; flex-direction: column;">
                    <div class="card-title">
                        ...
                        <small class="text-center"> {{ Auth::user()->nombre }} </div>
                        <small class="text-center"> {{ Auth::user()->usuario }} </div>
                    </div>
                    <form action="{{ route('updatePassword', ['id' => Auth::user()->id]) }}" method="post"
                        ...
                        <div class="form-floating  mb-3" style=" display: flex; flex-direction: column;">
                            ...
                            <small class="text" id="minimolargo">• Minimo 4 caracteres</div>
                            <small class="text" id="minimonumero">• Al menos 1 numero</div>
                            <small class="text" id="minimoletra">• Al menos 1 letra</div>
                        </div>
                        <button type="button" id="updateBoton" class="btn btn-outline-primary"
                            @click="sendForm">Actualizar contraseña</button>
                    </form>
                </div>
            </div>
        </div>
